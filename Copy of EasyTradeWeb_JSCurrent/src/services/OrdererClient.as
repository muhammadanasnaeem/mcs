package services
{
	...
	public class OrdererClient extends ServiceClient
	{
		...
		public function submitQuickOrder(orderBO:OrderBO):void
		{
			var webService:WebService=createService(Constants.ORDERER_WSDL_END_POINT);

			if (orderBO.SIDE == "buy")
			{
				webService.addEventListener("result", ModelManager.getInstance().orderModel.onBuyQuickOrderResult);
			}
			else
			{
				webService.addEventListener("result", ModelManager.getInstance().orderModel.onSellQuickOrderResult);
			}
			webService.addEventListener("fault", ModelManager.getInstance().orderModel.onFault);
