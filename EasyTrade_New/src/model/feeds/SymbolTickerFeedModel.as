package model.feeds
{
	...
	public class SymbolTickerFeedModel
	{
		...
		public function addFeedItem(code:String, change:Number, lastPrice:Number, lastVolume:Number):void
		{
			...
			tickerItemData.info=tickerItemData.CODE + " " + tickerItemData.lastPrice + "/" + tickerItemData.lastVolume + " ";
			...
			for each (var tid:TickerItemData in feed)
			{
				if (tid.guid == tickerItemData.guid)
				{
					...
					tid.info=tickerItemData.CODE + " " + ezCurrencyFormatter.format(tickerItemData.lastPrice) + "/" + ezNumberFormatter.format(tickerItemData.lastVolume) + " ";
					hasItem=true;
					break;
				}
			}
